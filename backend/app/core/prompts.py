...
4. **팩트 중심**: 있는 그대로만 서술
5. **[필수] 이슈명 복사-붙여넣기**: ### 뒤에 오는 이슈명은 위 목록의 **굵은 글씨(볼드)** 이슈명을 **한 글자도 바꾸지 말고 그대로** 복사해서 사용하세요
   - ✓ 올바름: `### 김도읍 정책위의장 사퇴` (위 목록에 있는 그대로)
   - ✗ 틀림: `### 김도읍 국민의힘 정책위의장 사퇴` (임의로 수정함)
6. **위 목록에 없는 이슈는 작성 금지**: 제공된 이슈 목록에 있는 것만 작성
